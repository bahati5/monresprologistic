...
$('#form_prealert').submit(function (event) {
    ...
    $.ajax({
        type: 'POST',
        url: '/logistics.monrespro.com/ajax/check_tracking.php',
        data: { tracking: formData.get('tracking_prealert') },
        dataType: 'json',
        success: function (response) {
            console.log('Tracking check response:', response);
            if (response.status === 'error') {
                ...
                console.log('Tracking check success:', response);
                // Si el número de seguimiento no está en uso, enviar el formulario
                $.ajax({
                    type: 'POST',
                    url: '/logistics.monrespro.com/ajax/pre_alerts/prealert_add_ajax.php',
                    data: formData,
                    contentType: false,
                    cache: false,
                    processData: false,
                    beforeSend: function () {
                        console.log('Sending prealert data...');
                        Swal.fire({
                            title: 'Process...',
                    ...
                    success: function (response) {
                        console.log('Prealert add response:', response);
                    ...
                    error: function (xhr, status, error) {
                        console.log('Prealert add error:', status, error);
                        console.log('Response text:', xhr.responseText);
                        Swal.fire({
                            type: 'error',
                            title: 'Error!',
                            text: message_error_form93 + ' (Status: ' + status + ')',
                        });
                    }
                });
            }
        ...
        error: function (xhr, status, error) {
            console.log('Tracking check error:', status, error);
            console.log('Response text:', xhr.responseText);
            Swal.fire({
                type: 'error',
                title: 'Error!',
