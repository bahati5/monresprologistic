...
var consolidate_id = $("#order_id").val();
var track_order = $("#track_order").val();

var order_sender_id = $("#order_sender_id").val();

function redirect() {
  window.location = "consolidate_package_view.php?id=" + consolidate_id;
}
...
function cdp_addPaymentPayPalSuccess(details) {
  $.ajax({
    url:
      "./ajax/consolidate_packages/add_payment_paypal_method_ajax.php?order_id=" +
      consolidate_id +
      "&track_order=" +
      track_order +
      "&customer=" +
...
if (active_stripe == 1) {
  ...
  card.on("change", function (event) {
    ...
    document.querySelector("#card-error-custom").textContent = event.error
      ? event.error.message
      : "";
  });
  ...
  form.addEventListener("submit", function (event) {
    ...
    fetch("./ajax/consolidate_packages/add_payment_stripe_method_ajax.php", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        email_property_card_stripe: document.getElementById(
          "email_property_card_stripe"
        ).value,
        name_property_card_stripe: document.getElementById(
          "name_property_card_stripe"
        ).value,
  ...
  function cdp_addPaymentStripeSuccess(details) {
    $.ajax({
      url:
        "./ajax/consolidate_packages/add_payment_stripe_method_success_ajax.php?order_id=" +
        consolidate_id +
        "&track_order=" +
        track_order +
        "&customer=" +
...
$("#add_charges").on("submit", function (event) {
  ...
  $.ajax({
    type: "POST",
    url:
      "./ajax/consolidate_packages/consolidate_add_payment_doc_ajax.php?order_id=" +
      consolidate_id,
    data: data,
...
if (active_paystack == 1) {
  ...
  order_total_order_paystack = parseFloat(
    order_total_order_paystack * 100
  ).toFixed(2);
  ...
  function cdp_addPaymentPaystackSuccess(details) {
    ...
    $.ajax({
      url:
        "./ajax/consolidate_packages/add_payment_paystack_method_success_ajax.php?order_id=" +
        consolidate_id +
        "&track_order=" +
        track_order +
        "&customer=" +
