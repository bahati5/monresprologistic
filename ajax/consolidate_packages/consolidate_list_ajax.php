...
$sql = "SELECT   a.status_invoice, a.total_order, a.consolidate_id , a.c_prefix, a.c_no, a.c_date, a.sender_id, a.receiver_id, a.order_courier, a.order_pay_mode, a.status_courier, a.driver_id, a.order_service_options,  b.mod_style, b.color FROM cdb_consolidate_packages as a
						...
						<tr class="card-hovera">
							...
							<?php if ($row->status_invoice == 2) { ?>
								<?php if ($userData->userlevel == 1) { ?>
									<td class="text-center">
									<a style="background: #34e89e;" class="label label" href="add_payment_gateways_consolidate_package.php?id_order=<?php echo $row->consolidate_id; ?>">
							...
							<td align='center'>
								<div class="btn-group">
									...
									<div class="dropdown-menu" style="overflow-y: auto; max-height: 200px;">
										...
										<?php if ($row->status_invoice == 2) { ?>
											<?php if ($userData->userlevel == 1) { ?>
												<a class="dropdown-item" href="add_payment_gateways_consolidate_package.php?id_order=<?php echo $row->consolidate_id; ?>">
													<i style="color:#343a40" class="fas fa-dollar-sign"></i>
										...
										<?php if ($userData->userlevel != 1) { ?>
											...
											<?php if ($userData->userlevel == 9 || $userData->userlevel == 2) { ?>
												<?php if ($row->status_courier != 8) { ?>
													...
													<a class="dropdown-item" href="consolidate_package_edit.php?id=<?php echo $row->consolidate_id; ?>" title="<?php echo $lang['langs_01029'] ?>">
										...
										<?php if ($row->status_courier != 21) { ?>
											...
											<a class="dropdown-item" target="blank" href="print_consolidate_package.php?id=<?php echo $row->consolidate_id; ?>"> <i style="color:#343a40" class="ti-printer"></i>&nbsp;<?php echo $lang['toolprint'] ?></a>
											<a class="dropdown-item" href="print_label_consolidate_package.php?id=<?php echo $row->consolidate_id; ?>" target="_blank"> <i style="color:#343a40" class="ti-printer"></i>&nbsp;<?php echo $lang['toollabel'] ?> </a>
											<?php if ($userData->userlevel != 1) { ?>

												<?php if ($row->status_courier != 8) { ?>

													<a class="dropdown-item" href="consolidate_package_shipment_tracking.php?id=<?php echo $row->consolidate_id; ?>" title="<?php echo $lang['toolupdate'] ?>"><i style="color:#20c997" class="ti-reload">&nbsp;</i><?php echo $lang['toolupdate'] ?></a>

													<a class="dropdown-item" href="consolidate_package_deliver_shipment.php?id=<?php echo $row->consolidate_id; ?>" title="<?php echo $lang['tooldeliver'] ?>"><i style="color:#2962FF" class="ti-package"></i>&nbsp;<?php echo $lang['tooldeliver'] ?></a>
