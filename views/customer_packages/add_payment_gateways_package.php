...
$agency_coords = [];
if (!empty($userData->name_off) && is_numeric($userData->name_off)) {
    $agency_coords = cdp_getAgencyPaymentCoordinates((int)$userData->name_off);
}
...
<body>
  ...
  <?php if ($active_stripe->is_active == 1) { ?>
  <script src="https://js.stripe.com/v3/"></script>
  <?php } ?>

  <?php if ($active_paypal->is_active == 1 && !empty($active_paypal->paypal_client_id)) { ?>
  ...
  <?php if ($active_paystack->is_active == 1) { ?>
                  ...
                  <div class="col-lg-12 mx-auto">
                    ...
                    <div class="card ">
                      <div class="card-header">
                        <div class="shadow-sm pt-3 pl-3 pr-3 pb-1" style="background-color: #3e5569 !important;">
                          ...
                          <ul role="tablist" class="nav bg-light nav-pills rounded nav-fill mb-3">
                            ...
                            ?>
                              <li class="nav-item">
                                <a data-toggle="pill" data-turbo="false" href="#stripe" class="nav-link <?php if ($first_tab) { echo 'active'; $first_tab = false; } ?>">
                            ...
                            ?>
                              <li class="nav-item">
                                <a data-toggle="pill" data-turbo="false" href="#paypal" class="nav-link <?php if ($first_tab) { echo 'active'; $first_tab = false; } ?>">
                            ...
                            ?>
                              ...
                              <li class="nav-item">
                                <a data-toggle="pill" data-turbo="false" href="#paystack" class="nav-link <?php if ($first_tab) { echo 'active'; $first_tab = false; } ?>">
                            ...
                            ?>
                              ...
                              <li class="nav-item">
                                <a data-toggle="pill" data-turbo="false" href="#attach" class="nav-link <?php if ($first_tab) { echo 'active'; $first_tab = false; } ?>">
                      ...
                      <div class="tab-content bg-white">
                          ...
                          if ($active_stripe->is_active == 1) {
                          ?>
                            <div id="stripe" class="tab-pane fade <?php if ($first_pane) { echo 'show active'; $first_pane = false; } ?> pt-3">
                          ...
                          ?>
                            ...
                            <!-- PAYPAL TAB-PANE -->
                            <div id="paypal" class="tab-pane fade <?php if ($first_pane) { echo 'show active'; $first_pane = false; } ?> pt-3">
                          ...
                          ?>
                            ...
                            <!-- PAYSTACK TAB-PANE -->
                            <div id="paystack" class="tab-pane fade <?php if ($first_pane) { echo 'show active'; $first_pane = false; } ?> pt-3">
                          ...
                          ?>
                            ...
                            <!-- ATTACH TAB-PANE -->
                            <div id="attach" class="tab-pane fade <?php if ($first_pane) { echo 'show active'; $first_pane = false; } ?> pt-3">
  ...
  <script>
  // DEBUG: Force Bootstrap tab pane visibility on page load
  $(document).ready(function(){
      var tc = document.querySelector('.tab-content');
      var panes = document.querySelectorAll('.tab-pane');
      console.log('[TAB DEBUG] tab-content found:', !!tc);
      console.log('[TAB DEBUG] tab-panes count:', panes.length);
      panes.forEach(function(p){
          var cs = window.getComputedStyle(p);
          console.log('[TAB DEBUG] pane#' + p.id, 'classes:', p.className, 'display:', cs.display, 'opacity:', cs.opacity, 'visibility:', cs.visibility, 'height:', cs.height);
      });
      // Force the first active pane visible
      var active = document.querySelector('.tab-pane.active') || document.querySelector('.tab-pane');
      if (active) {
          active.classList.add('show', 'active');
          active.style.cssText = 'display:block!important;opacity:1!important;visibility:visible!important;';
          console.log('[TAB DEBUG] Forced visible:', active.id);
      }
      // Also check ancestors for overflow:hidden
      if (active) {
          var el = active.parentElement;
          while (el && el !== document.body) {
              var s = window.getComputedStyle(el);
              if (s.overflow === 'hidden' || s.overflowY === 'hidden' || s.maxHeight === '0px' || s.height === '0px') {
                  console.log('[TAB DEBUG] ANCESTOR BLOCKING:', el.tagName, el.className, 'overflow:', s.overflow, 'height:', s.height, 'maxHeight:', s.maxHeight);
              }
              el = el.parentElement;
          }
      }
