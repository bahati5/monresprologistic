...
// ===========================================================
// MONRESPRO — Coordonnées agences
// ===========================================================

function cdp_getAgencyPaymentCoordinates($branch_id)
{
    $db = new Conexion;
    $db->cdp_query('SELECT * FROM cdb_agency_payment_coordinates WHERE branch_id = :branch_id AND is_active = 1 ORDER BY sort_order, id');
    $db->bind(':branch_id', $branch_id);
    $db->cdp_execute();
    return $db->cdp_registros();
}

function cdp_getAllAgencyPaymentCoordinates()
{
    $db = new Conexion;
    $db->cdp_query('SELECT c.*, b.name_branch FROM cdb_agency_payment_coordinates c LEFT JOIN cdb_branchoffices b ON c.branch_id = b.id ORDER BY b.name_branch, c.sort_order, c.id');
    $db->cdp_execute();
    return $db->cdp_registros();
}

function cdp_insertAgencyPaymentCoordinate($datos)
{
    $db = new Conexion;
    $db->cdp_query('INSERT INTO cdb_agency_payment_coordinates (branch_id, label, account_identifier, currency, is_active, sort_order) VALUES (:branch_id, :label, :account_identifier, :currency, 1, :sort_order)');
    $db->bind(':branch_id', $datos['branch_id']);
    $db->bind(':label', $datos['label']);
    $db->bind(':account_identifier', $datos['account_identifier']);
    $db->bind(':currency', isset($datos['currency']) ? $datos['currency'] : null);
    $db->bind(':sort_order', isset($datos['sort_order']) ? $datos['sort_order'] : 0);
    $db->cdp_execute();
    return $db->dbh->lastInsertId();
}

function cdp_updateAgencyPaymentCoordinate($id, $datos)
{
    $db = new Conexion;
    $db->cdp_query('UPDATE cdb_agency_payment_coordinates SET branch_id = :branch_id, label = :label, account_identifier = :account_identifier, currency = :currency, sort_order = :sort_order, is_active = :is_active WHERE id = :id');
    $db->bind(':branch_id', $datos['branch_id']);
    $db->bind(':label', $datos['label']);
    $db->bind(':account_identifier', $datos['account_identifier']);
    $db->bind(':currency', isset($datos['currency']) ? $datos['currency'] : null);
    $db->bind(':sort_order', isset($datos['sort_order']) ? $datos['sort_order'] : 0);
    $db->bind(':is_active', isset($datos['is_active']) ? $datos['is_active'] : 1);
    $db->bind(':id', $id);
    return $db->cdp_execute();
}

function cdp_deleteAgencyPaymentCoordinate($id)
{
    $db = new Conexion;
    $db->cdp_query('DELETE FROM cdb_agency_payment_coordinates WHERE id = :id');
    $db->bind(':id', $id);
    return $db->cdp_execute();
}
